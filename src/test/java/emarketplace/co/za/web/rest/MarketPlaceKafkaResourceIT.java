...
import emarketplace.co.za.config.KafkaProperties;
...
class MarketPlaceKafkaResourceIT {
    ...
    void setup() {
        ...
        MarketPlaceKafkaResource kafkaResource = new MarketPlaceKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc.perform(post("/api/market-place-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc.perform(get("/api/market-place-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
